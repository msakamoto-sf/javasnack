 ...
 */
public class TestRe2jBasicEscapeQuoteDemo {
    @Test
    ...
    public void quoteDemo() {
        // re2j では Pattern#quote() が \Q-\E ではなく、正規表現としての記号について \ でエスケープする。
        //assertThat("\\Q\\d{3}\\E").isEqualTo(Pattern.quote("\\d{3}"));
        assertThat("\\\\d\\{3\\}").isEqualTo(Pattern.quote("\\d{3}"));
        //assertThat("\\Q(?im)(^abc.*def$)\\E").isEqualTo(Pattern.quote("(?im)(^abc.*def$)"));
        assertThat("\\(\\?im\\)\\(\\^abc\\.\\*def\\$\\)").isEqualTo(Pattern.quote("(?im)(^abc.*def$)"));
    }
}
